...
let get_decls_and_symbols (env : Env.t) : ((FuncDecl.func_decl * Symbol.symbol) list) =
  ...
  let var_to_decl ~key:_ ~data:z3_var decls =
    ...
    let decl = FuncDecl.mk_const_decl_s ctx
        (Expr.to_string z3_var |> (String.strip ~drop:(fun c -> c = '|')))
        (Expr.get_sort z3_var) in
    let sym =  Symbol.mk_string ctx (Expr.to_string z3_var |> (String.strip ~drop:(fun c -> c = '|'))) in
    (decl, sym) :: decls
  in
...
let check_external 
  ...
  let (solver_stdout, solver_stdin) = Unix.open_process solver_path in
  ...
  in
  if (result = "unsat") then (* Regexp it? *)
    Z3.Solver.UNSATISFIABLE
  else if (result = "sat") then
    begin
      let () = process_sat () in
      Z3.Solver.SATISFIABLE
    end
